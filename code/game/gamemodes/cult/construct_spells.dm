...
/spell/targeted/fortify/cast(list/targets, mob/living/user)
	if(findNullRod(user) || user.has_legacy_modifier_of_type(/datum/legacy_modifier/fortify))
		charge_counter = 400
		return
	user.add_legacy_modifier(/datum/legacy_modifier/fortify, 1 MINUTES)
...
/spell/targeted/occult_repair_aura/cast(list/targets, mob/living/user)
	if(findNullRod(user) || user.has_legacy_modifier_of_type(/datum/legacy_modifier/repair_aura))
		charge_counter = 300
		return
	user.add_legacy_modifier(/datum/legacy_modifier/repair_aura, 30 SECONDS)
...
/spell/targeted/ambush_mode/cast(list/targets, mob/living/user)
	if(findNullRod(user))
		charge_counter = 50
		return
	if(user.has_legacy_modifier_of_type(/datum/legacy_modifier/ambush))
		user.remove_legacy_modifiers_of_type(/datum/legacy_modifier/ambush)
		return
	user.add_legacy_modifier(/datum/legacy_modifier/ambush, 0)
...
/obj/item/weapon/spell/construct/spawner/agonizing_sphere/on_throw_cast(atom/hit_atom, mob/user)
	...
	if(isliving(hit_atom))
		...
		L.add_legacy_modifier(/datum/legacy_modifier/agonize, 10 SECONDS)
...
/obj/effect/temporary_effect/pulse/agonizing_sphere/on_pulse()
	for(var/mob/living/L in view(4,src))
		if(!iscultist(L) && !istype(L, /mob/living/simple_mob/construct))
			L.add_legacy_modifier(/datum/legacy_modifier/agonize, 2 SECONDS)
			if(L.isSynthetic())
...
/obj/item/weapon/spell/construct/mend_occult/on_melee_cast(atom/hit_atom, mob/living/user, def_zone)
	if(isliving(hit_atom))
		var/mob/living/L = hit_atom
		L.add_legacy_modifier(/datum/legacy_modifier/mend_occult, 150)
	qdel(src)
