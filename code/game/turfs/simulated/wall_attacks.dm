...
/turf/simulated/wall/proc/try_touch(var/mob/user, var/rotting)
	...
	if(rotting)
		if(reinf_material)
			...
			to_chat(user, "<span class='danger'>\The [material_legacy.display_name] crumbles under your touch!</span>")
			dismantle_wall()
			return 1

	if(!can_open)
		if(!material_legacy.wall_touch_special(src, user))
...
/turf/simulated/wall/attack_hand(var/mob/user)
	...
	if (HULK in user.mutations)
		if (rotting || !prob(material_legacy.hardness))
			success_smash(user)
		else
			fail_smash(user)
			return 1
...
/turf/simulated/wall/attack_generic(var/mob/user, var/damage, var/attack_message)
	...
	if(reinf_material)
		if(damage >= max(material_legacy.hardness, reinf_material.hardness) )
			return success_smash(user)
	else if(damage >= material_legacy.hardness)
		return success_smash(user)
...
/turf/simulated/wall/attackby(var/obj/item/weapon/W, var/mob/user)
	...
	if(isnull(construction_stage) || !reinf_material)
		...
		var/cut_delay = 60 - material_legacy.cut_delay
		var/dismantle_verb
		var/dismantle_sound
