...
/obj/item/weapon/stool/New(var/newloc, var/new_material, var/new_padding_material)
	...
	if(!istype(material_legacy))
		qdel(src)
		return
	force = round(material_legacy.get_blunt_damage()*0.4)
	update_icon()
...
/obj/item/weapon/stool/update_icon()
	...
	cut_overlays()
	// Base icon.
	var/cache_key = "stool-[material_legacy.name]"
	...
	if(padding_material)
		...
		desc = "A padded stool. Apply butt. It's made of [material_legacy.use_name] and covered with [padding_material.use_name]."
	else
		name = "[material_legacy.display_name] [initial(name)]"
		desc = "A stool. Apply butt with care. It's made of [material_legacy.use_name]."
...
/obj/item/weapon/stool/proc/dismantle()
	if(material_legacy)
		material_legacy.place_sheet(get_turf(src))
	if(padding_material)
		padding_material.place_sheet(get_turf(src))
	qdel(src)
...
/obj/item/weapon/stool/attackby(obj/item/weapon/W as obj, mob/user as mob)
	if(W.is_wrench())
		...
		if(istype(W,/obj/item/stack/tile/carpet))
			...
			if(M.material_legacy && (M.material_legacy.flags & MATERIAL_PADDING))
				padding_type = "[M.material_legacy.name]"
		if(!padding_type)
