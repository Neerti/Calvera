...
/obj/structure/simple_door/proc/TemperatureAct(temperature)
	hardness -= material_legacy.combustion_effect(get_turf(src),temperature, 0.3)
	CheckHardness()
...
/obj/structure/simple_door/New(var/newloc, var/material_name)
	...
	if(!material_legacy)
		qdel(src)
		return
	hardness = max(1,round(material_legacy.integrity/10))
	icon_state = material_legacy.door_icon_base
	name = "[material_legacy.display_name] door"
	color = material_legacy.icon_colour
	if(material_legacy.opacity < 0.5)
		set_opacity(0)
	else
		set_opacity(1)
...
/obj/structure/simple_door/proc/TryToSwitchState(atom/user)
	...
	if(ismob(user))
		var/mob/M = user
		if(!material_legacy.can_open_material_door(user))
			return
...
/obj/structure/simple_door/proc/Open()
	isSwitchingStates = 1
	playsound(src, material_legacy.dooropen_noise, 100, 1)
	flick("[material_legacy.door_icon_base]opening",src)
	sleep(10)
	density = 0
	set_opacity(0)
	state = 1
...
/obj/structure/simple_door/proc/Close()
	isSwitchingStates = 1
	playsound(src, material_legacy.dooropen_noise, 100, 1)
	flick("[material_legacy.door_icon_base]closing",src)
	sleep(10)
	density = 1
	set_opacity(1)
	state = 0
...
/obj/structure/simple_door/update_icon()
	if(state)
		icon_state = "[material_legacy.door_icon_base]open"
	else
		icon_state = material_legacy.door_icon_base
...
/obj/structure/simple_door/process()
	if(!material_legacy.radioactivity)
		return
	SSradiation.radiate(src, round(material_legacy.radioactivity/3))
