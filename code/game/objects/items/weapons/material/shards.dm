...
/obj/item/weapon/material/shard/set_legacy_material(var/new_material)
	...
	icon_state = "[legacy_material.shard_icon][pick("large", "medium", "small")]"
	randpixel_xy()
	update_icon()

	if(legacy_material.shard_type)
		name = "[legacy_material.display_name] [legacy_material.shard_type]"
		desc = "A small piece of [legacy_material.display_name]. It looks sharp, you wouldn't want to step on it barefoot. Could probably be used as ... a throwing weapon?"
		switch(legacy_material.shard_type)
...
/obj/item/weapon/material/shard/on_update_icon()
	if(legacy_material)
		...
		alpha = 255 * (1 - (1 - legacy_material.opacity)*(1 - legacy_material.opacity))
	else
		color = "#ffffff"
		alpha = 255

/obj/item/weapon/material/shard/attackby(obj/item/weapon/W as obj, mob/user as mob)
	if(istype(W, /obj/item/weapon/weldingtool) && legacy_material.shard_can_repair)
