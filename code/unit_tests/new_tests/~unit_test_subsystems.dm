...
SUBSYSTEM_DEF(unit_tests)
	name = "Unit Tests"
	wait = 2 SECONDS
	init_order = INIT_ORDER_UNIT_TESTS
...
/datum/controller/subsystem/unit_tests/proc/start_game()
	if(CURRENT_RUNLEVEL >= RUNLEVEL_POSTGAME)
		log_unit_test("Unable to start testing - game is finished!")
		del world
		return

	if((CURRENT_RUNLEVEL == RUNLEVEL_LOBBY) && !SSticker.start_now())
...
/datum/controller/subsystem/unit_tests/proc/await_game_running()
	if(CURRENT_RUNLEVEL == RUNLEVEL_GAME)
