...
/obj/item/organ/internal/spleen/handle_germ_effects()
	...
	if (. >= 1)
		if(prob(1))
			...
			owner.add_legacy_modifier(/datum/legacy_modifier/trait/haemophilia, 2 MINUTES * spleen_efficiency)
	if (. >= 2)
		if(prob(1))
...
/obj/item/organ/internal/spleen/die()
	..()
	if(owner)
		owner.add_legacy_modifier(/datum/legacy_modifier/trait/haemophilia, round(15 MINUTES * spleen_efficiency))
