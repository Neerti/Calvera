...
/obj/machinery/particle_smasher/update_icon()
	...
	if(anchored)
		...
		if(target)
			material_layer.color = target.material_legacy.icon_colour
			add_overlay(material_layer)
			if(successful_craft)
				material_glow.color = target.material_legacy.icon_colour
	...
	if(target && energy)
		...
		light_color = target.material_legacy.icon_colour
