...
/obj/structure/table/proc/straight_table_check(var/direction)
	...
	for(var/angle in list(-90,90))
		...
		if(T && T.flipped == 0 && T.legacy_material.name == legacy_material.name)
			return 0
	T = locate() in get_step(src.loc,direction)
	if (!T || T.flipped == 1 || T.legacy_material != legacy_material)
		return 1
...
/obj/structure/table/proc/unflipping_check(var/direction)
	...
	for(var/new_dir in L)
		...
		if(T && T.legacy_material.name == legacy_material.name)
...
/obj/structure/table/proc/flip(var/direction)
	...
	for(var/D in list(turn(direction, 90), turn(direction, -90)))
		...
		if(T && T.flipped == 0 && legacy_material && T.legacy_material && T.legacy_material.name == legacy_material.name)
			T.flip(direction)
