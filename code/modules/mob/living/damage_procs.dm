...
/mob/living/proc/apply_effects(var/stun = 0, var/weaken = 0, var/paralyze = 0, var/irradiate = 0, var/stutter = 0, var/eyeblur = 0, var/drowsy = 0, var/agony = 0, var/blocked = 0, var/ignite = 0, var/flammable = 0)
	...
	if(ignite)
		if(ignite >= 3)
			add_legacy_modifier(/datum/legacy_modifier/fire/stack_managed/intense, 60 SECONDS)
		else
			add_legacy_modifier(/datum/legacy_modifier/fire/stack_managed, 45 * ignite SECONDS)
	return 1
