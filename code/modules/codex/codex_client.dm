	...
	if(LAZYLEN(all_entries) == 1)
		...
		if(LAZYLEN(all_entries) > 1)
			...
			for(var/i = 1 to min(all_entries.len, max_codex_entries_shown))
				...
				codex_data += "<tr><td>[entry.name]</td><td>"
				codex_data += href(SScodex, list("show_examined_info" = entry, "show_to" = mob), "View")
				codex_data += "</td></tr>"
			codex_data += "</table>"
...
/client/verb/list_codex_entries()
	...
	if(!mob || !SScodex.subsystem_initialized)
		return
	...
	for(var/thing in SScodex.index_file)
		...
		if(first_letter != last_first_letter)
			...
			codex_data += "<tr><td colspan = 2><h2>[last_first_letter]</h2></td></tr>"
			codex_data += "<tr><td colspan = 2><hr></td></tr>"
		codex_data += "<tr><td>[thing]</td><td>"
		codex_data += href(SScodex, list("show_examined_info" = SScodex.index_file[thing], "show_to" = mob), "View")
		codex_data += "</td></tr>"
	codex_data += "</table>"
	popup.set_content(jointext(codex_data, null))
	popup.open()
