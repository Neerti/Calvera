			...
			if(linked_destroy.busy)
				...
				spawn(2.4 SECONDS)
					if(linked_destroy)
						...
						if(linked_lathe && linked_destroy.loaded_item.matter_legacy) // Also sends salvaged materials to a linked protolathe, if any.
							for(var/t in linked_destroy.loaded_item.matter_legacy)
								if(t in linked_lathe.materials)
									linked_lathe.materials[t] += min(linked_lathe.max_material_storage - linked_lathe.TotalMaterials(), linked_destroy.loaded_item.matter_legacy[t] * linked_destroy.decon_mod)
